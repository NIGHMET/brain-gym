    ...
    start(root, onDone, setStatus) {
      ...
      function resolvePair() {
        ...
        if (a.dataset.sym === b.dataset.sym) {
          ...
          SFX.good();
          if (matched === N_PAIRS) {
            const winner = score[0] === score[1] ? 0 : (score[0] > score[1] ? 0 : 1);
            const tie = score[0] === score[1];
            const scoreVal = U.clamp(Math.round((score[winner] / N_PAIRS) * 100 + (tie ? 50 : 0)), 40, 100);
            setStatus(tie ? '平局！' : `玩家 ${turn === 0 ? 'A' : 'B'} 获胜！`);
            timer = setTimeout(() => onDone({
              score: scoreVal,
              detail: tie ? `平局 ${score[0]} : ${score[1]}` : `A ${score[0]} : B ${score[1]}，${turn === 0 ? 'A' : 'B'} 获胜`,
              extra: { duel: true, winner: tie ? -1 : turn, sA: score[0], sB: score[1] }
            }), 700);
          }
        } else {
          lock = true;
